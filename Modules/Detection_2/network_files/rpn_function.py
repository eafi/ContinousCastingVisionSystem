...
class AnchorsGenerator(nn.Module):
    ...
    def num_anchors_per_location(self):
        ...
        # size: anchor box的预设尺寸
        # aspect_ratios: 预设比例
        # 如果是ResNet+FPN -> [3,3,3,3,3]
        # 如果是MobileNet -> [15]
    ...
    def forward(self, image_list, feature_maps):
        ...
        # 获取每个预测特征层的尺寸(height, width), 注意ResNet+FPN的多feature maps输出
        # 0 - 大分辨率 --------> 4 - 小分辨率
        # 小anchor box检查 ----  大anchor box检查
        grid_sizes = list([feature_map.shape[-2:] for feature_map in feature_maps])

        # 获取输入图像的height和width \
        # 注意，一个batched images只有一个size， 因为这些图片都经过: 拉伸+batch处理了
        ...
        # 计算特征层上的一步等于batch后图像尺寸上的步长
        # 举例： strides x=4: 说明这个特征图上每一个像素的距离，等价于batched images上的4个像素距离
        # 举例： strides x=61: 说明这个特征图上每一个像素的距离，等价于batched images上的61个像素距离
        # 显然，stride越小，意味着在原图上移动的越精细
        ...
        # 得到的是一个list列表，对应每张预测特征图映射回原图的anchors坐标
        ...
        # anchors是个list，每个元素为一张图像的所有anchors信息, len(anchors): 一个batch中图片的数量(batch_size)
...
class RPNHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors):
        ...
        :param in_channels:
        :param num_anchors:  每一个特征图的cell产生多少个anchor box？
        """
        super(RPNHead, self).__init__()
        # 3x3 滑动窗口
        self.conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
        # 计算预测的目标分数（这里的目标只是指前景或者背景）
        # num_anchors意味着这个位置输出的num_anchors个值，每一个值代表对应anchor box的目标分数
    ...
    def forward(self, x):
        ...
        logits = []
        bbox_reg = []
        for i, feature in enumerate(x): # ResNet+FPN有五层输出特征层
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        Args：
            anchors: (List[Tensor]) 一个batch中每个图片产生的所有anchors （非常多）
            targets: (List[Dict[Tensor]) 一个batch中每个图片的所有GT信息（非常少）
        Returns:
            labels: 标记anchors归属类别（1, 0, -1分别对应正样本，背景，废弃的样本）
                    注意，在RPN中只有前景和背景，所有正样本的类别都是1，0代表背景
            matched_gt_boxes：与anchors匹配的gt

        NOTE: 返回值labels和matched_gt_boxes并不是说要将海量的anchors压缩到与targets数量一致且IOU最匹配， \
                而是anchors的维度保持不变(非常多), **用target去匹配anchors**
                简单来说，本函数的返回值的长度等于anchor boxes的长度， 尽管targets数量很少.
        """
        labels = []
        matched_gt_boxes = []
        # 遍历每张图像的anchors和targets
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            gt_boxes = targets_per_image["boxes"]  # 只要bbox标签
            if gt_boxes.numel() == 0:
                ...
                """
                match_quality_matrix 维度为 (num_GT_bbox, num_anchor_box)
                """
                match_quality_matrix = box_ops.box_iou(gt_boxes, anchors_per_image)
                # 计算每个anchors与gt匹配iou最大的索引（如果iou<0.3索引置为-1，0.3<iou<0.7索引为-2）
                """
                matched_idxs: 长度为anchor boxes的长度
                里面的数值为 -1 : 负样本    -2 ： 丢弃的样本  >=0 ：这个anchor匹配到的对应GT box的索引
                """
                ...
                matched_gt_boxes_per_image = gt_boxes[matched_idxs.clamp(min=0)]  # NOTE: -2 -1 这种值变成了0
            ...
            """
            labels_per_image : 长度为 num_anchor， 1为正样本位置,-1为负，-2为丢弃
            matched_gt_boxes_per_image: 长度同样为num_anchor, 每一个位置对应的就是这个位置的anchor匹配的GT索引
            """
    ...
    def _get_top_n_idx(self, objectness, num_anchors_per_level):
        ...
        Args:
            objectness: Tensor(每张图像的预测目标概率信息 ), 是将所有的feature map容和在一起的(mobilenet没有这个问题)
        ...
        for ob in objectness.split(num_anchors_per_level, 1):
            ...
            # 如果是reset+FPN那么就会有多个特征层，但是此处的排序是单独针对每一个特征层，因此需要加上特征层的偏移
            # 才是正确的索引.
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        objectness = objectness.reshape(num_images, -1)  # 按照batch size展开
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        Arguments:
            ...
            pred_bbox_deltas (Tensor)：预测的bbox regression，**没有解码的**，神经网络直接输出的值
            labels (List[Tensor])：真实的标签 1, 0, -1（batch中每一张图片的labels对应List的一个元素中）
            regression_targets (List[Tensor])：**真实的bbox regression，但是被encode了，向anchor进行了encode**
        ...
        # 将所有正负样本索引拼接在一起 每256一组为一个图片的训练样本
    ...
    def forward(self,
        ...
        # 生成一个batch图像的所有anchors信息,list(anchors)元素个数等于batch_size
        # 四个图对应了自己的anchor， （而且这些anchor完全一摸一样）
        ...
        # 调整内部tensor格式以及shape
        # 输出结果相当于把batch维度flatten，将所有图片统一处理
        objectness, pred_bbox_deltas = concat_box_prediction_layers(objectness,
                                                                    pred_bbox_deltas)

        # 将预测的bbox regression参数应用到anchors上得到最终预测bbox坐标
        # NOTE: 关键细节： RPN中产生的proposals并没有用于RPN训练，而只是用于Faster RCNN部分！！ \
        #  原因也是容易理解的： 如果将神经网络产生的proposals用于RPN训练，那么就成了自己训练自己了！！
        ...
        if self.training:
            ...
            # NOTE： 这里的传入参数是anchors，并非经过filter后的boxes！！
            # 也就是说这里的anchors数量还是非常庞大的那个没有经过神经网络调整的那个.
