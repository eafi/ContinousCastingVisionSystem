...
def encode_boxes(reference_boxes, proposals, weights):
    ...
    Arguments:
        reference_boxes (Tensor): reference boxes(gt)  GT boxes
        proposals (Tensor): boxes to be encoded(anchors)  Anchor boxes
        weights:
    """
    ...
    targets_dx = wx * (gt_ctr_x - ex_ctr_x) / (ex_widths + 1e-6)
    targets_dy = wy * (gt_ctr_y - ex_ctr_y) / (ex_heights + 1e-6)
    targets_dw = ww * torch.log((gt_widths+1e-6) / (ex_widths+1e-6))
    targets_dh = wh * torch.log((gt_heights+1e-6) / (ex_heights+1e-6))
...
class BoxCoder(object):
    ...
    def decode(self, rel_codes, boxes):
        ...
        Args:
            rel_codes: bbox regression parameters 来自Head网络的bbox调整预测参数
            boxes: anchors/proposals， 是没有经过调整的anchor
        ...
        # 到这里，rel_codes同boxes一样，将没有batch size维度，所有的图片全部压倒了同一个维度，变成2维数据
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        # 网络预测的是小数，而在下面修正过程时需要使用anchor box先验宽度（像素宽度）乘以小数，再加上像素距离
...
class Matcher(object):
    ...
    def __call__(self, match_quality_matrix):
        ...
        """
        match_quality_matrix 为MxN矩阵，M代表本图像中的GT box数量； N代表所有的anchor box
        NOTE： 操作仅针对单张图片进行!
        让每一个anchor box比较每一个GT box，找出最大值
        ====
        |   |Anchor1|Anchor2|Anchor3|...|
        |GT1|0.2    |**0.5**|**0.7**|...|
        |GT2|**0.8**| 0.1   | 0.2   |...|
        结果：
        1. matched_vals: 代表这个Anchor最匹配GT box的最大值 Nx1
        2. matches： 这个Anchor列的最匹配GT box是哪一个, 取值范围为0~num_GT-1
        
        细节点1. matches代表的是某一个anchor的最匹配GT box的索引
        """
        # matches对应最大值所在的索引
        ...
        # 在论文中讨论到，可能存在一些GT，没有一个anchor与他们的IOU大于0.7，
        # 可以通过这个操作，让这些anchors中尽管小于0.7，但最大IOU的那一个anchor置为正样
        if self.allow_low_quality_matches:
    ...
    def set_low_quality_matches_(self, matches, all_matches, match_quality_matrix):
        ...
        """
        之前是将每一个anchor box比较所有GT，选择最大的哪一个GT(dim=0)， \
        现在是将每一个GT比较所有的anchor boxes，选择最大的那一个anchor
        
        1. 维度变为 Mx1 (这是一个非常小的维度，因为GT boxes数量远小于anchor数量)
        2. 每一个值代表这个GT box从大量anchor boxes中能够找到的最大IOU
        """
