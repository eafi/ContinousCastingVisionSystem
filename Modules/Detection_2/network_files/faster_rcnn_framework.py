...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        # 对图像进行预处理,注意并非pytorch.transform, 是GeneralizedRCNNTransform class
        # 注意，这里右边的images输入来自Dataset，是尺寸完全没变化的图片
        # 左边的images为List, 保存了batch后的images以及拉伸后的images尺寸（与original_image_size保存的不同）
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 # transform parameter
                 min_size=768, max_size=768*2,      # 预处理resize时限制的最小尺寸与最大尺寸
        ...
        """
        如果是MobileNet做backbone，那么Backbone最后一层只有一个尺度的feature maps；
        如果是ResNet+FPN做backbone，最后输出的是5层不同尺度的feature maps，
        因为五种feature maps(List)刚好对应不同尺度，刚好对应了不同大小anchor box的含义：
        大anchor box找大目标； 小anchor box找小目标。
        NOTE： anchor box的尺寸是针对原图像尺寸来说的.
        """
        if rpn_anchor_generator is None:
            anchor_sizes = ((32,), (64,), (128,), (256,), (512,))
            aspect_ratios = ((0.5, 1.0, 2.0),) * len(anchor_sizes)  # ResNet+FPN在一个cell位置产生5*3=15anchor boxes
